<?php
/**
 * ThemeIsle Customizer Notification System.
 *
 * @package ThemeIsle
 */
 ...
 */
class Themeisle_Customizer_Notify {
	...
	/**
	 * The single instance of Themeisle_Customizer_Notify
	 *
	 * @var Themeisle_Customizer_Notify $instance The Themeisle_Customizer_Notify instance.
	 */
	...
	/**
	 * The Main Themeisle_Customizer_Notify instance.
	 *
	 * We make sure that only one instance of Themeisle_Customizer_Notify exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Themeisle_Customizer_Notify ) ) {
			self::$instance = new Themeisle_Customizer_Notify;
	...
	public function setup_actions() {
		...
		add_action( 'customize_register', array( $this, 'themeisle_customizer_notify_customize_register' ) );

		// Enqueue scripts and styles
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'themeisle_customizer_notify_scripts_for_customizer' ), 0 );

		/* ajax callback for dismissable recommended actions */
		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_action', array( $this, 'themeisle_customizer_notify_dismiss_recommended_action_callback' ) );

		add_action( 'wp_ajax_ti_customizer_notify_dismiss_recommended_plugins', array( $this, 'themeisle_customizer_notify_dismiss_recommended_plugins_callback' ) );
	...
	public function themeisle_customizer_notify_scripts_for_customizer() {
		...
		wp_localize_script(
			'ti-customizer-notify-customizer-js', 'tiCustomizerNotifyObject', array(
				...
				'base_path'                => admin_url(),
				'activating_string'        => __( 'Activating', 'hestia-pro' ),
			)
		);
	 ...
	 */
	public function themeisle_customizer_notify_customize_register( $wp_customize ) {

		/**
		 * Include the Themeisle_Customizer_Notify_Section class.
		 */
		require_once get_template_directory() . '/ti-customizer-notify/class-themeisle-customizer-notify-section.php';

		$wp_customize->register_section_type( 'Themeisle_Customizer_Notify_Section' );

		$wp_customize->add_section(
			new Themeisle_Customizer_Notify_Section(
				$wp_customize,
	 ...
	 */
	public function themeisle_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_option( 'themeisle_customizer_notify_show_recommended_actions' ) ) {

				$ti_customizer_notify_show_recommended_actions = get_option( 'themeisle_customizer_notify_show_recommended_actions' );
				switch ( $_GET['todo'] ) {
					case 'add':
				...
				update_option( 'themeisle_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions );
				...
				if ( ! empty( $ti_customizer_notify_recommended_actions ) ) {
					...
					update_option( 'themeisle_customizer_notify_show_recommended_actions', $ti_customizer_notify_show_recommended_actions_new );
				}
			}
		}
	 ...
	 */
	public function themeisle_customizer_notify_dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			/* if the option exists, update the record for the specified id */
			$ti_customizer_notify_show_recommended_plugins = get_option( 'themeisle_customizer_notify_show_recommended_plugins' );
			...
			update_option( 'themeisle_customizer_notify_show_recommended_plugins', $ti_customizer_notify_show_recommended_plugins );
		}
		die(); // this is required to return a proper result
	}
