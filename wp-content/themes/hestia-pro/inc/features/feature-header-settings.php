...
function hestia_header_customize_register( $wp_customize ) {
	...
	$wp_customize->add_section(
		'hestia_top_bar', array(
			'title'    => esc_html__( 'Top Bar', 'hestia-pro' ),
	...
	if ( ! empty( $top_bar_sidebar ) ) {
		...
		$hestia_top_bar_hide = $wp_customize->get_control( 'hestia_top_bar_hide' );
		if ( ! empty( $hestia_top_bar_hide ) ) {
			$hestia_top_bar_hide->section = 'sidebar-widgets-sidebar-top-bar';
			$hestia_top_bar_hide->priority = -2;
		}
	}
