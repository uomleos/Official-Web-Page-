	...
	public function render_content() {
		...
		if ( $this->id === 'hestia_subscribe_info' ) {
			$path    =
				...
				sprintf(
					'<b>%s</b>',
					esc_html__( 'Customize > Widgets > Subscribe Section', 'hestia-pro' )
				);
		...
		if ( $this->id === 'hestia_blog_subscribe_info' ) {
			$path    =
				...
				sprintf(
					'<b>%s</b>',
					esc_html__( 'Customize > Widgets > Blog Subscribe Section', 'hestia-pro' )
				);
		...
		printf(
			...
			esc_html__( 'The main content of this section is customizable in: %1$s. There you must add the "SendinBlue Newsletter" widget. But first you will need to install %2$s.', 'hestia-pro' ),
			$path,
			esc_html__( 'SendinBlue plugin', 'hestia-pro' )
		);
		...
		echo '<br/><br/>';
		echo esc_html__( 'And then you need to navigate to its Settings, and use the following in the Subscription form:', 'hestia-pro' );
		echo '<br/><br/>';
