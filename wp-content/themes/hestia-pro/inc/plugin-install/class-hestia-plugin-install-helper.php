...
class Hestia_Plugin_Install_Helper {
		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				case 'install':
					...
					$button .= '<a data-slug="' . $slug . '" class="install-now hestia-install-plugin button  " href="' . esc_url( $nonce ) . '" data-name="' . $slug . '" aria-label="Install ' . $slug . '">' . __( 'Install and activate', 'hestia-pro' ) . '</a>';
					break;
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			...
			array(
				'activating' => esc_html__( 'Activating ', 'hestia-pro' ),
			)
		);
		wp_localize_script(
