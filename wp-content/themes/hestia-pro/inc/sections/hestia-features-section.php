...
if ( ! function_exists( 'hestia_features' ) ) :
	...
	function hestia_features( $is_shortcode = false ) {
		...
		if ( current_user_can( 'edit_theme_options' ) ) {
			$default_title = esc_html__( 'Why our product is the best', 'hestia-pro' );
		...
		if ( ! $is_shortcode && ((is_single() && ( (bool) $show_features_single_product === false)) || (is_front_page() && ( (bool) $hide_section === true))) || ((empty( $hestia_features_title )) && (empty( $hestia_features_subtitle )) && (empty( $hestia_features_content ))) ) {
			return;
		}
