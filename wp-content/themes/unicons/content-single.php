				...
				<div class="large-9 columns <?php if ( !is_active_sidebar( 'sidebar' ) ){ ?> nosid <?php }?> ">
						...
						<!--Content-->
   						<?php if(have_posts()): ?><?php while(have_posts()): ?><?php the_post(); ?>
                                                         ...
                                                         $separator = ' ';
 														 $output = '';
															if ( ! empty( $categories ) ) {
        														...
        														$output .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr(sprintf( esc_attr__( 'View all posts in %s', 						                 'unicons' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>' . $separator;
    																}
								...
								<?php comments_popup_link( esc_attr__('0 Comment', 'unicons'), esc_attr__('1 Comment', 'unicons'), esc_attr__('% Comments', 'unicons'), '', esc_attr__('Off' , 'unicons')); ?>
					...
					<div class="post_content wow fadeIn">
						...
						<?php wp_link_pages( array(
								'before'      => '<div class="page-links"><span class="page-links-title">' . esc_attr__( 'Pages:', 'unicons' ) . '</span>',
								'after'       => '</div>',
								'link_before' => '<span>',
								'link_after'  => '</span>',
								'pagelink'    => '<span class="screen-reader-text">' . esc_attr__( 'Page', 'unicons' ) . ' </span>%',
